...
namespace ESI.DAL
{
    public class ESI_KPIConfigurationDAL2
    {
        public static SuccessMessage SaveKpiConfigurationData(List<KPIViewModel> MainKPI, List<SubKPIViewModel> SubKPI, List<ConditionViewModel> Condition, int sChannelId, int year, int quarter, int thresholdType, int userId, string userName)
        {
            ...
            try
            {
                int ReportCycleId = SaveReportCycle(sChannelId, year, quarter, thresholdType, createdBy, transaction);
                if (ReportCycleId > 0)
                {
                    isValid = true;
                    int lastConditionId = 0;
                    for (int MonthNo = 1; MonthNo <= QuarterMonth; MonthNo++)
                    {
        ...
        public static int SaveReportCycle(int sChannelId, int year, int quarter, int thresholdType, int createdBy, ESI_DBTransaction transaction)
        {
            ...
            procedure.AddInputParameter("pThresholdType", thresholdType, OracleType.Number);
        ...
        public static int UpdateConditionConfiguration(int monthId, int UpdatedBy, int ReportCycleId, ESI_DBTransaction transaction)
        {
            int ReturnValue = 0;
        ...
        public static SuccessMessage UpdateKpiConfigurationData(List<KPIViewModel> MainKPI, List<SubKPIViewModel> SubKPI, List<ConditionViewModel> Condition, int ReportCycleId, int Month, int UserId, string userName)
        {
            ...
            try
            {
                ...
                {
                    returnValue = UpdateConditionConfiguration(i, updatedBy, ReportCycleId, transaction);
                }
                ...
                {
                    isValid = false;
                    ReturnMessage.Message = "Some condition is not valid!!";
                    ReturnMessage.Type = (int)Message.MessageType.Fail;
                }
        ...
        public static SuccessMessage dataValidation(List<KPIViewModel> MainKPI, List<SubKPIViewModel> SubKPI, List<ConditionViewModel> Condition)
        {
            ...
            //TUQuIFJBS0lCIEhBU0FO SubKPI.GroupBy(p => p.Kpi_id).Sum(m=>m.;
        }
