...
{
    public class DenoCampaignDAL
    {

        public static List<DenoCampaignEnt> GetItemList(int Id)
        {
            OracleProcedure procedure = new OracleProcedure(Utility.GetSchemaSetup(), "GET_DENOCAMPAIGNSETUP");
            procedure.AddInputParameter("PCAMPAIGN_ID", Id, OracleType.Number);

            try
            {
                DataTable dt = procedure.ExecuteQueryToDataTable();
                List<DenoCampaignEnt> results = new List<DenoCampaignEnt>();
                foreach (DataRow dr in dt.Rows)
                {
                    results.Add(new DenoCampaignEnt(dr));
                }
        ...
        public static List<DenoCampaignEnt> GetActiveItemList(int Id)
        {
            OracleProcedure procedure = new OracleProcedure(Utility.GetSchemaSetup(), "GET_ACTIVEDENOCAMPAIGN");
            procedure.AddInputParameter("PCAMPAIGN_ID", Id, OracleType.Number);
        ...
        {
            OracleProcedure procedure = new OracleProcedure(Utility.GetSchemaSetup(), "INACTIVE_DENO_CAM");
        ...
        {
            OracleProcedure procedure = new OracleProcedure(Utility.GetSchemaSetup(), "GENERATE_DENO_CAM_TARGET");
            procedure.AddInputParameter("PCAMPAIGN_ID", campaignId, OracleType.Number);
