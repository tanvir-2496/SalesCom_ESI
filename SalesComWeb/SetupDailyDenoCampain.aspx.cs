...
public partial class SetupDailyDenoCampain : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            if (!Permissions.DailyCampaignView)
            {
    ...
    {
        List<DailyDenoCampaignEnt> list = DailyDenoCampaignDAL.GetItemList(0);
        ...
        if (e.CommandName == "DownloadTarget")
        {
            ...
            try
            {
                if (arg != null)
                {
                    ...
                    DataTable dt_excel = DailyDenoCampaignDAL.DownloadCampaignTarget(CampaignID);
    ...
    protected void lv_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        if (e.Item.ItemType == ListViewItemType.DataItem)
        {
            ...
            if (lbtn != null)
            {
                ...
                DateTime endDate = Convert.ToDateTime(DataBinder.Eval(dataitem.DataItem, "CampaignEndDate"));
                if (isActive == "N" && DateTime.Now.Date <= endDate && Permissions.DailyCampaignUpdate)
                {
                    lbtn.Visible = true;
                }
                else
    ...
    private int StopCampaign(int campaignId)
    {
        try
        {
            ...
            int ErrorCode = DailyDenoCampaignDAL.InactiveCampaign(campaignId, update_by);
