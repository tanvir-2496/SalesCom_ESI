    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (!string.IsNullOrEmpty(Request["Id"]))
            {
                ...
                double camount = 0;
                double wamount = 0;
                double.TryParse(lblCommissionAmt.Text, out camount);
                double.TryParse(lblWithheldAmount.Text, out wamount);
                lblPaybaleAmout.Text = (camount - wamount).ToString();
                this.lblApprovalLevelName.Text = LevelName;

                List<CommentsCycleEnt> temp = PendingApprovalWithStatusDAL.GetPreviousComment(ApprovalflowId, int.Parse(CycleId));
