...
<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPages/Modal.master" AutoEventWireup="true" CodeFile="CampaignDenoDriveSetupAdd.aspx.cs" Inherits="CampaignDenoDriveSetupAdd" %>
...
<style>
    ...
    select[readonly] {
     ...
     <script type="text/javascript">
         function EditRecipientTypeChange(modalityValue) {
             ...
             if (recipientTypeId != "" && recipientTypeId != 0) {
                 ...
                 $.ajax({
                     ...
                     success: function (data) {
                         ...
                         var currentVal = parseInt(modalityValue);
                         modalityTypeDDL.val(currentVal);
                     },
                     error: function (er) {
             ...
             if (mode == "edit") {
                 ...
                 if (modalityType != "" && modalityType != 0) {
                     if (modalityType == 1) {
                         ...
                         if (greaterStatus == 1) {
                             document.getElementById("<%=chkgreaterEqual.ClientID%>").checked = true;
                             $(".clsSlab1").hide();
                         }
                 ...
                 EditRecipientTypeChange(modalityType);
         ...
         function DenoAddValidation() {
             ...
             if (DenoAmount != "" && DenoAmount != 0) {
                 DenoAmount = parseInt(DenoAmount, 10);
                 if (getDeno != "") {
         ...
         function AddDeno() {
             ...
             var isChecked = document.getElementById("<%=chkgreaterEqual.ClientID%>").checked;
             ...
             if (denoStart != "" && denoEnd != "" && denoIncentive != "") {
                 ...
                 if (isChecked == false) {
                     if (parseInt(denoEnd) > parseInt(denoStart)) {
                         ...
                         if (getLength >= 1) {
                             var indexP = getLength - 2;
                             var EndValue = $(".dummy_item_denoend").children('input[name="DenoInfo[' + indexP + '].DenoEnd"]').val();

                             var currentEndValue = parseInt(EndValue);
                             var currentdenoStart = parseInt(denoStart);

                             if (currentEndValue >= currentdenoStart) {
                                 alert("Start slab should be greater than previous End slab.");
                             } else {
                                 ...
                                 tableRow = tableRow + "<td class='dummy_item_denostatus' style='display: none'><input type='hidden' name='DenoInfo[" + (rowSL) + "].DenoStatus' value='" + status + "'> <span>" + status + "</span></td>";
         ...
         function RemoveThisDenoItem(selector) {
             ...
             var getStatus = $(selector).closest("td").parent("tr").find('.dummy_item_denostatus').find('input').val();

             if (getStatus == 1) {
                 greaterStatus = 0;
                 $(selector).closest("td").parent("tr").remove();
                 $(".clsSlab1").show();
                 document.getElementById("<%=chkgreaterEqual.ClientID%>").checked = false;
                 document.getElementById("<%=txtSlabStart.ClientID%>").value = "";
                 document.getElementById("<%=txtSlabEnd.ClientID%>").value = "";
                 document.getElementById("<%=txtSlabInceptive.ClientID%>").value = "";
                 document.getElementById("<%=txtSlabEnd.ClientID%>").readOnly = false;
                 document.getElementById("<%=txtSlabEnd.ClientID%>").classList.remove("clsReadOnly");
             }
             else {
                 $(selector).closest("td").parent("tr").remove();
             }
         ...
         function GreaterThanEqual() {
             ...
             if (isChecked == true) {
                 ...
                 document.getElementById("<%=txtSlabEnd.ClientID%>").readOnly = true;
                 document.getElementById("<%=txtSlabEnd.ClientID%>").value = 0;
                 document.getElementById("<%=txtSlabEnd.ClientID%>").classList.add("clsReadOnly");
             } else {
             ...
             if (isValid == true) {
                 ...
                 if (modality == "1") {
                     ...
                     $(".dummy_row_line_item").each(function (index, value) {
                         ...
                         var thisStatus = $(thisRow).find("td").children('input[name="DenoInfo[' + index + '].DenoStatus"]').val();
             ...
             $.ajax({
                 ...
                 success: function (data) {
                     ...
                     if (data.d == "SUCCESSFUL" || data.d == "SUCCE") {
                         ...
                         var mode = document.getElementById("<%=hdnEditMode.ClientID%>").value;
                         if (mode == "edit") {
                             $("span[id$=lblMsg]").html("<b>Campaign data process successfully update.</b><br>").css("color", "green");
                         } else {
         ...
         function GetCheckBoxStatus() {
             ...
             function IsStartDateValid(startDate) {
                 ...
                 if (startDate != "") {
                     ...
                     var ConfigValue = document.getElementById("<%=hdnConfigValue.ClientID%>").value;
                 // alert(ConfigValue);
                 var getDateRange = (30 + parseInt(ConfigValue));
                 var validDateRange = -(30 + parseInt(ConfigValue));
                 if (diffInDays >= validDateRange) {
                     return true;
                 } else {
                     $("span[id$=lblMsg]").html("<b>Campaign start date out of date range.</b><br>").css("color", "red");
                     //$("span[id$=lblMsg]").html("<b>Campaign start date must be minimum today and onwards.</b><br>").css("color", "red");
                     return false;
                 }
             }
         ...
         function IsEndDateValid(startDate, endDate) {
             ...
             if (startDate != "" && endDate != "") {
                 ...
                 if (diffInDays >= 0) {
                     ...
                     var SecondCurrentDay = new Date().getDate();         // Return the day as a number (1-31)
                     var SecondCurrentMonth = new Date().getMonth() + 1;           // Return current month (1-12)
                     var SecondCurrentYear = new Date().getFullYear();

                     var SecondcStartDate = SecondCurrentYear + "-" + SecondCurrentMonth + "-" + SecondCurrentDay;
                     var SecondgetStartDate = new Date(SecondcStartDate);

                     var SecondcEndDate = iYear + "-" + iMonth + "-" + iDate;
                     var SecondgetEndDate = new Date(SecondcEndDate);

                     var SeconddiffInMs = SecondgetStartDate - SecondgetEndDate;

                     var ConfigValue = document.getElementById("<%=hdnConfigValue.ClientID%>").value;

                     var validDateRange = (parseInt(ConfigValue));
                     var SeconddiffInDays = SeconddiffInMs / (1000 * 60 * 60 * 24);


                     if (SeconddiffInDays > validDateRange) {
                         $("span[id$=lblMsg]").html("<b>Campaign end date must be minimum " + validDateRange + " past date and onwards.</b><br>").css("color", "red");
                         return false;
                     }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <asp:HiddenField runat="server" ID="hdnConfigValue" />
    ...
    <uc2:WebUserControl ID="WebUserControl1" runat="server" />
        ...
        <tr>
            ...
            <td>
                <asp:TextBox  ID="txtAmount" CssClass="required" runat="server" SkinID="txtAmount" onkeydown="return (!(event.keyCode>=65) && event.keyCode!=32);">
                </asp:TextBox>
        ...
        <tr>
            ...
            <td>
                <asp:TextBox ID="txtDenoAmount" ReadOnly="false" CssClass="required" disabled="true" style="background-color: #E0E0E0 !important;" runat="server" SkinID="txtDenoAmount">
                </asp:TextBox>
          ...
          <tr>
            ...
            <td>
                <asp:DropDownList ID="ddlModality" onchange="ModalityChange()" runat="server" SkinID="ddlCommonSkin" CssClass="required"></asp:DropDownList>
          ...
          <tr class="clsSlab">
            <td>
             ...
             <td>
                <asp:CheckBox runat="server" name="clsCheckbox" ID="chkHitTarget" onChange="HitTarget()" Checked="false" />
            </td>
        </tr>
        ...
        <tr class="clsSlab">
            <td>
             ...
             <td>
                <asp:CheckBox runat="server" name="clsCheckbox" ID="chkAchivement" onChange="HitAchivement()" Checked="false" />
            </td>
        </tr>
            ...
            <td>
                <asp:CheckBox runat="server" name="clsCheckbox" ID="chkPercentage" onChange="Percentage()" Checked="false" /><asp:TextBox ID="txtPercentage" style="height: 22px !important; width: 125px !important;" ReadOnly="true" CssClass="required clsPercentage" runat="server" SkinID="txtCommonSkin" onkeydown="return (!(event.keyCode>=65) && event.keyCode!=32);">
                </asp:TextBox> %
            </td>
        </tr>
            ...
            <td>
                <asp:CheckBox runat="server" name="clsCheckbox" onChange="Incentive()" ID="chkIncentive" Checked="false" /><asp:TextBox ID="txtIncentive" style="height: 22px !important; width: 125px !important;" ReadOnly="true" CssClass="required clsIncentive" runat="server" SkinID="txtCommonSkin" onkeydown="return (!(event.keyCode>=65) && event.keyCode!=32);">
                </asp:TextBox>
            </td>
        </tr>
            ...
            <td>
                <asp:CheckBox runat="server" name="clsCheckbox" onChange="MaxCap()" ID="chkMaxCap" Checked="false" /><asp:TextBox ID="txtMaxCap" style="height: 22px !important; width: 125px !important;" ReadOnly="true" CssClass="required clsMaxCap" runat="server" SkinID="txtCommonSkin" onkeydown="return (!(event.keyCode>=65) && event.keyCode!=32);">
                </asp:TextBox> %
            </td>
        </tr>
        ...
        <tr class="clsCondition clsAchivement clsParticipation">
            ...
            <td>
               <asp:CheckBox runat="server" name="clsCheckbox" onChange="OverHitAmount()" ID="chkHitAmount" Checked="false" /> 
            </td>
        </tr>
       ...
       <tr class="clsCondition clsAchivement clsParticipation">
            ...
            <td>
               <asp:CheckBox runat="server" name="clsCheckbox" onChange="PlaneHitAmount()" ID="chkPlaneHitAmount" Checked="false" /> 
            </td>
        </tr>
     ...
     <tr class="clsCondition clsAchivement clsParticipation">
            ...
            <td>
               <asp:TextBox ID="txtHitAmount" style="height: 22px !important; width: 125px !important;margin-left: 20px !important;" ReadOnly="true" CssClass="required clsHitAmount"  runat="server" SkinID="txtCommonSkin" onkeydown="return (!(event.keyCode>=65) && event.keyCode!=32);">
                </asp:TextBox>
            </td>
        </tr>
    ...
    <tr class="clsSlab">
        <td>
        <asp:Label ID="greaterEqual" runat="server" Text="Is Greater Than or Equal"> </asp:Label>
        </td>

        <td>
        <asp:CheckBox runat="server" name="clsCheckbox" CssClass="clschkgreaterEqual" onChange="GreaterThanEqual()" ID="chkgreaterEqual" Checked="false" /> 
        </td>
    </tr>
                            ...
                            <div class="clsSlab1">
                                 ...
                                 <tr>
                                    ...
                                    <td>
                                      ...
                                      <td>
                                        <asp:Label ID="lblSlabInceptive" runat="server" Text="Incentive Amount"> </asp:Label>
                                    </td>
                                    <td>
                                     ...
                                     <td>
                                      <asp:Button ID="Button2" OnClientClick="return AddDeno();" style="background-color: #FCDCC5 !important;" CssClass="slabAdd" runat="server" Text="Add" SkinID="btnCommonSkin" />
                                 </td>
                                 </tr>
                            </table>
                            </div>                           
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            var mode = document.getElementById("<%=hdnEditMode.ClientID%>").value;
            var slabStatus = document.getElementById("<%=hdnStatus.ClientID%>").value;



            Onload();
            if (mode == "edit") {
                greaterStatus = parseInt(slabStatus);
            }
