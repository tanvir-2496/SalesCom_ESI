...
public partial class PendingApproval : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            if (!Permissions.ClaimApprovalView)
            {
    ...
    {
        List<ClaimApprovalProcessEnt> list;
        if (commissionCycleId > 0)
        {
            list = ClaimApprovalProcessDAL.GetItemList(commissionCycleId, LoginInfo.Current.UserId);
        }
        else
        {
            list = new List<ClaimApprovalProcessEnt>();
        }
        lv.DataSource = list;
        lv.DataBind();
    ...
    protected void lv_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        ...
        try
        {
            Common.ExportToExcel(dt_excel, String.Format("Claim_Details_{0}", System.DateTime.Now.ToString("ddMMyyy-HHmmss")));
        }
        catch (Exception ex)
        {
