<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPages/default.master" AutoEventWireup="true" CodeFile="SetupReportApprovalProcess.aspx.cs" Inherits="SetupReportApprovalProcess" %>
                            ...
                            <tr class='<%# Container.DataItemIndex % 2 == 0 ? "row" : "altrow" %>' style="width: 100%">
                                ...
                                <td style="text-align: center">
                                    <% if (Permissions.ReportApprovalAct)
                                       { %>
                                    <a type="button" href="SetupReportApprovalAct.aspx?ID= <%#Eval("report_approval_id")%>&RN=<%#Eval("REPORT_NAME") %>&ALN=<%#Eval("approvallevelname") %>&AF=<%#Eval("approval_flow_id") %>&AL=<%#Eval("approval_level_id") %>&OI=<%#Eval("orderid") %>&TB_iframe=true&height=400&width=650"
