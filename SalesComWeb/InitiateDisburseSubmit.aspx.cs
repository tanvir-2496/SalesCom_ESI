...
public partial class InitiateClaimSubmit : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            if (!Permissions.InitiateDisburseApprovalUpload)
            {
            ...
            if (!String.IsNullOrEmpty(Request["RCID"]))
            {
                ...
                this.lblClaimAmount.Text = Request.QueryString["CA"];
            }
        }
    ...
    {
        Common.Download("Channel Withheld List.xlsx", "ImportFileSample/Channel Withheld List.xlsx");
    }
    ...
    private void LoadDataToGrid()
    {
        ...
        //{
        //    this.lblResult.ForeColor = Color.Red;
        //    this.lblResult.Text = "Please select a worksheet!";
        //}
    ...
    private void Reset()
    {
        this.lblResult.Text = String.Empty;
        this.lblResult.ForeColor = Color.SteelBlue;
        //this.ddlSheets.DataSource = null;
    ...
    private void SubmitDataToDataBase()
    {
        if (DataLoaded)
        {
            if (this.gvLoadData.Rows.Count > 0)
            {
                if (DuplicateRow == false)
                {
                    ...
                    List<ClaimMismatchEnt> mismatchData = InitiateDisburseDAL.SaveWithheldList(table, ReportCycleId);
                    if (mismatchData.Count == 0)
                    {
                        this.lblResult.ForeColor = Color.SteelBlue;
                        this.lblResult.Text = InitiateDisburseDAL.SetWithheldList(ReportCycleId, lblSetReportname.Text, ReportFlow, ReportDuration, this.lblClaimAmount.Text, LoginInfo.Current.UserId, LoginInfo.Current.UserName, "YES");
        ...
        {
            this.lblResult.ForeColor = Color.SteelBlue;
            this.lblResult.Text = InitiateDisburseDAL.SetWithheldList(ReportCycleId, lblSetReportname.Text, ReportFlow, ReportDuration, this.lblClaimAmount.Text, LoginInfo.Current.UserId, LoginInfo.Current.UserName, "NO");
            if (lblResult.Text == "Data Successfully Updated")
            {
                this.plDataGrid.Visible = false;
                ScriptManager.RegisterStartupScript(this, this.GetType(), "refresh", "parent.refreshWindow();", true);
