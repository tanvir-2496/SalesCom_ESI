...
public partial class DenoDriveTargetUpload : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (!Permissions.CampaignDenoTarget)
            {
            ...
            Common.PopulateDenoCampignList(ddlReportName);
        ...
        if (true)//CheckInput()
        {
            ...
            if (ddlSheets.SelectedIndex != 0 && ddlSheets.SelectedValue != null)
            {
                ...
                if (this.rbHDR.SelectedValue == "Yes")
                {
                    ...
                    {
                        this.lblResult.ForeColor = Color.Red;
                        this.lblResult.Text = "Max limit is 4 columns.";
                    }
                }
                else
                {
                    ...
                    {
                        this.lblResult.ForeColor = Color.Red;
                        this.lblResult.Text = "Your Selected sheet contains 5 columns but the max is 4 columns.";
                    }
                }
            }
            else
    ...
    private void ImportData(DataTable dtExcelRecords, int campaignId)
    {
        try
        {
            if (checkThresholdMismatch(dtExcelRecords))
            {
                ...
                var message = DenoDriveTargetUploadDAL.ImportDatatoDatabaseTabel(dtExcelRecords, campaignId, imported_by);
    ...
    {
        Common.Download("DenoDriveCampTargetSample.xlsx", "ImportFileSample/DenoDriveCampTargetSample.xlsx");
    }
}
