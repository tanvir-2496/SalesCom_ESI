...
public partial class KPIDetailReportView : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            if (!Permissions.ESIKPIReportView)
            {
    ...
    private void BindData(int userId, int salesGroup, int reportType, int channelId, int year, int quarter, int month)
    {
        try
        {
            ...
            list = kpi_approval_dal.GetItemList(userId, salesGroup, reportType, channelId, year, quarter, month, "ESI_GETRPTCYCLEBYSCHIDDETAIL");
            //List<kpi_approval_ent> newlist = list.FindAll(x => x.current_level == "Approved By All");
            lv.DataSource = list;
            lv.DataBind();
    ...
    protected void lv_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        ...
        try
        {
            DataTable dt_excel = ESI_ReportDAL.DetailReportData(CycleReportID);
