<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPages/default.master" AutoEventWireup="true" CodeFile="DisburseReportList.aspx.cs" Inherits="PendingApproval" %>
                        ...
                        <ItemTemplate>
                            ...
                            <tr class='<%# Container.DataItemIndex % 2 == 0 ? "row" : "altrow" %>'>
                                ...
                                <td style="text-align: center">
                                    ...
                                    <%if (Permissions.DisburseReportDownload)
                                      { %>
                                    <asp:LinkButton ID="lbtnDetailsAmount" runat="server" CommandName="DetailsAmount" CommandArgument='<%#Eval("REPORT_CYCLE_ID") +"|"+ Eval("REPORTNAME")+"|"+Eval("REPORT_DURATION")%>' Text="Download" />
                                    <%} else { %>
                                    <span>disallowed</span>
                                    <%} %>
                                </td>
                                <td style="text-align: center">
                                    <%if (Permissions.PosUploadReportDownload)
                                      { %>
                                    <asp:LinkButton ID="lbtnPosUploadDetails" runat="server" CommandName="PosUploadDetails" CommandArgument='<%#Eval("REPORT_CYCLE_ID") +"|"+ Eval("REPORTNAME")+"|"+Eval("REPORT_DURATION")%>' Text="Download" />
                                    <%} else { %>
                                    <span>disallowed</span>
                                    <%} %>
                                </td>
